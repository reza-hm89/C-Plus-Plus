...
void InputData()
{
	...
	{
		cout<<"\nInput EnterTime && ServiceTime moshtari "<<i+1<<" : ";
		cin>>t1>>t2;
	...
	while (1)
	{
		for (int i=0;i<NumOfConsumer;i++)
		{
			...
			if (c[i].Get_EnterTime()==Time)
			{
				t[k].Consumer_ID=i;
				t[k].time=Time;
				strcpy(t[k].events,"vorood be arayeshgah");
				k++;
				...
				for (int j=0;j<NumOfStylist;j++)
				{
					...
					if (s[j].Get_CheckStatus()==false && c[i].Serviced==false)
					{
						...
						t[k].Consumer_ID=i;
						t[k].time=Time;
						strcpy(t[k].events,"shoroo be service");
						k++;
					}
				}
				...
				if (check==false && Time>=c[i].Get_EnterTime())
				{
					chair.Add(i);
					cnt++;
					strcpy( t[k].events , "ezafe be saaf");
					t[k].time = Time;
                    t[k++].Consumer_ID = i;
				}
			}
			...
			if (Time == (s[i].Get_Start() + s[i].Get_BusyTime()))
                    {
                        ...
                        if (cnt > 0)
                        {
                            ...
                            cnt--;
                            strcpy( t[k].events , "hazf az saf va shoroo be service");
							t[k].time = Time;
...
{
	float ave=0;
	cout<<"\nZamane entezare moshtariyan:\n";
	for (int i=0;i<NumOfConsumer;i++)
	{
		cout<<"Moshtari "<<i+1<<":"<<c[i].Get_WaitTime()<<"\n";
		ave+=c[i].Get_WaitTime();
	}

	cout<<"\n\nZamane Mashghool budane arayeshgaraa:\n";
	for (int i=0;i<NumOfStylist;i++)
	{
		cout<<"Arayeshgare "<<i+1<<":"<<s[i].Get_TotalBusyTime()<<"\n";
	}

	cout<<"\n\nZamane bikari arayeshgaraaa:\n";
	for (int i=0;i<NumOfStylist;i++)
	{
		cout<<"Arayeshgare "<<i+1<<":"<<s[i].Get_FreeTime()<<"\n";
	}
